    ...
    <div style={{ background: '#0A0A0A', minHeight: '100vh', color: '#F0EEE8', fontFamily: "'DM Sans', sans-serif", overflowX: 'hidden' }}>
      <style>{`
        ...
        @media(max-width:768px) {
          ...
          .preview-cols { grid-template-columns:repeat(2,1fr) !important; }
        }
      `}</style>
      ...
      {/* HERO */}
      <section style={{ position:'relative', padding:'6rem 2rem 4rem', maxWidth:900, margin:'0 auto', textAlign:'center' }}>
        ...
        <div className="fade-up" style={{ animationDelay:'0.1s', display:'inline-flex', alignItems:'center', gap:8, background:'rgba(29,158,117,0.1)', border:'0.5px solid rgba(29,158,117,0.25)', borderRadius:20, padding:'6px 14px', fontSize:12, color:'#1D9E75', marginBottom:'1.5rem' }}>
          ...
          Now live — start journaling for free
        </div>

        <h1 className="hero-title fade-up" style={{ fontFamily:"'Syne',sans-serif", fontSize:54, fontWeight:700, lineHeight:1.1, marginBottom:'1.5rem', animationDelay:'0.2s', letterSpacing:'-0.5px' }}>
          The trading journal<br />
          <span style={{ color:'#1D9E75' }}>built for serious traders</span>
        </h1>

        <p className="fade-up" style={{ fontSize:17, color:'rgba(240,238,232,0.5)', lineHeight:1.7, maxWidth:540, margin:'0 auto 2.5rem', animationDelay:'0.3s' }}>
        ...
        <div className="hero-btns fade-up" style={{ display:'flex', gap:12, justifyContent:'center', animationDelay:'0.4s' }}>
          <button className="cta-btn" onClick={() => navigate('/login')}>Start journaling free</button>
          <button className="outline-btn" onClick={() => document.getElementById('features').scrollIntoView({ behavior:'smooth' })}>See features</button>
        </div>

        {/* Dashboard preview */}
        <div className="fade-up" style={{ marginTop:'4rem', animationDelay:'0.5s', background:'#111', border:'0.5px solid rgba(255,255,255,0.08)', borderRadius:16, padding:'1.5rem', textAlign:'left', maxWidth:680, margin:'4rem auto 0' }}>
          <div className="preview-cols" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:'1.25rem' }}>
          ...
          {[['EUR/USD','Order block','TP hit'],['BTC/USD','Fair value gap','SL hit'],['XAU/USD','Break of structure','Open']].map(([pair,setup,result]) => (
            <div key={pair} style={{ display:'grid', gridTemplateColumns:'100px 1fr 80px', gap:8, padding:'10px 0', borderTop:'0.5px solid rgba(255,255,255,0.04)', fontSize:13, alignItems:'center' }}>
      ...
      <section id="stats" style={{ padding:'4rem 2rem', borderTop:'0.5px solid rgba(255,255,255,0.06)', borderBottom:'0.5px solid rgba(255,255,255,0.06)' }}>
        <div className="stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, maxWidth:800, margin:'0 auto', textAlign:'center' }}>
          {[['80%','of traders lose because they never review their trades'],['100%','free to start — no credit card needed'],['1-click','chart capture with the Chrome extension'],['No limits','on trades you can log — ever']].map(([num,desc]) => (
            <div key={num}>
      ...
      <section id="features" style={{ padding:'5rem 2rem', maxWidth:1000, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:'3rem' }}>
          ...
          <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:34, fontWeight:600, lineHeight:1.2 }}>Everything a trader needs.<br />Nothing they don't.</h2>
        </div>

        <div className="features-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
          {[
            { icon: <CameraIcon />, title:'Auto chart capture', desc:'Chrome extension captures your TradingView chart and trade data in one click. No more screenshots and copy-pasting.' },
            { icon: <BrainIcon />, title:'Emotion tracking', desc:'Log how you felt before every trade. FLOW reveals which emotional states lead to your best and worst results.' },
            { icon: <ChartIcon />, title:'Performance stats', desc:'Win rate, performance by pair, setup analysis and more. Know exactly what works and what does not.' },
            { icon: <SearchIcon />, title:'Trade review', desc:'Filter trades by pair, setup, emotion or result. Review your history with full context, chart and reasoning.' },
            { icon: <MobileIcon />, title:'Mobile friendly', desc:'Full access on your phone with a native-feel bottom navigation. Log trades and review your journal anywhere.' },
            { icon: <LockIcon />, title:'Your data, private', desc:'Every trade is private to your account. No one else can see your journal or your performance.' },
          ].map(({ icon, title, desc }) => (
      ...
      <section id="how" style={{ padding:'5rem 2rem', background:'#0D0D0D' }}>
        <div style={{ maxWidth:600, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:'3rem' }}>
            ...
            <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:34, fontWeight:600 }}>Three steps to a better trading mindset</h2>
          </div>
          {[
            ...
            { n:'2', title:'Let FLOW track results', desc:'When your trade closes, update the result in one tap — TP hit, SL hit, or manually closed. Your journal stays accurate with zero effort.' },
      ...
      <section style={{ padding:'6rem 2rem', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:38, fontWeight:700, marginBottom:'1rem', position:'relative' }}>Ready to trade smarter?</h2>
...
function CameraIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg> }
function BrainIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><path d="M9.5 2A2.5 2.5 0 0112 4.5v15a2.5 2.5 0 01-4.96-.46 2.5 2.5 0 01-1.07-4.69A3 3 0 016 9.5a3 3 0 012-2.83V6.5A2.5 2.5 0 019.5 2zM14.5 2A2.5 2.5 0 0112 4.5v15a2.5 2.5 0 004.96-.46 2.5 2.5 0 001.07-4.69A3 3 0 0018 9.5a3 3 0 00-2-2.83V6.5A2.5 2.5 0 0014.5 2z"/></svg> }
function ChartIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> }
function SearchIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> }
function MobileIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg> }
function LockIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1D9E75" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg> }
