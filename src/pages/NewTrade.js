...
export default function NewTrade({ session }) {
  ...
  const extractFromChart = async () => {
    ...
    try {
      ...
      const res = await fetch('https://lfbncazxxieqwcksjfpv.supabase.co/functions/v1/extract-chart', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding:'1.25rem', maxWidth:680, margin:'0 auto', paddingBottom:'5rem' }}>
      <style>{`
        ...
        .stitle{font-size:11px;font-weight:500;color:rgba(240,238,232,0.3);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:14px;}
        .fl{font-size:11px;color:rgba(240,238,232,0.4);margin-bottom:5px;display:block;}
        ...
        @media(max-width:480px){.g3{grid-template-columns:1fr 1fr !important;}}
      `}</style>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="section">
          <p className="stitle">Chart screenshot</p>
        ...
        {/* TRADE DETAILS */}
        <div className="section">
          <p className="stitle">Trade details</p>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div className="g2">
              <div><label className="fl">Date</label><input type="date" value={form.trade_date} onChange={e => set('trade_date', e.target.value)} /></div>
              <div><label className="fl">Time</label><input type="time" value={form.trade_time} onChange={e => set('trade_time', e.target.value)} /></div>
            </div>
            <div className="g2">
              <div><label className="fl">Pair / Ticker *</label><input value={form.pair} onChange={e => set('pair', e.target.value.toUpperCase())} placeholder="EUR/USD, BTC/USD..." /></div>
            ...
            <div className="g3">
              <div><label className="fl">Entry</label><input type="number" step="any" value={form.entry_price} onChange={e => set('entry_price', e.target.value)} placeholder="0.00" /></div>
              <div><label className="fl">Stop loss</label><input type="number" step="any" value={form.stop_loss} onChange={e => set('stop_loss', e.target.value)} placeholder="0.00" /></div>
              <div><label className="fl">Take profit</label><input type="number" step="any" value={form.take_profit} onChange={e => set('take_profit', e.target.value)} placeholder="0.00" /></div>
            </div>
            <div className="g3">
              <div><label className="fl">Risk : Reward</label><input value={form.risk_reward} onChange={e => set('risk_reward', e.target.value)} placeholder="1:2" /></div>
              <div><label className="fl">Session</label>
                <select value={form.session} onChange={e => set('session', e.target.value)}>
                  {SESSIONS.map(s => <option key={s} value={s}>{s || 'Select'}</option>)}
                </select>
              </div>
              <div><label className="fl">Result</label>
                <select value={form.result} onChange={e => set('result', e.target.value)}>
                  {RESULTS.map(r => <option key={r}>{r}</option>)}
                </select>
              </div>
            </div>
            <div><label className="fl">Setup type</label>
        ...
        {/* PSYCHOLOGY */}
        <div className="section">
          <p className="stitle">Psychology</p>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div className="g2">
              <div><label className="fl">A+ setup?</label>
              ...
              <div><label className="fl">Planned or FOMO?</label>
            ...
            <div className="g2">
              <div><label className="fl">Entry type</label>
              ...
              <div><label className="fl">Followed your rules?</label>
            ...
            <div>
              <label className="fl">Confidence level: {form.confidence_level}/10</label>
        ...
        {/* JOURNAL */}
        <div className="section">
          <p className="stitle">Journal</p>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div><label className="fl">Why did you take this trade?</label>
            ...
            <div><label className="fl">Mistakes / lessons learned</label>
